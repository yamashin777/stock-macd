...
def _sb_save(key, value):
    """Supabaseのsettingsテーブルにvalueをupsertする（PATCH→なければINSERT）"""
    try:
        ...
        if ins_resp.status_code not in (200, 201, 204):
            print(f'Supabase save error ({key}): HTTP {ins_resp.status_code} {ins_resp.text[:200]}')
            return False
        return True
    except Exception as e:
...
def save_watchlist(wl: list, profile: str = '') -> None:
    ...
    sb_ok = False
    if SUPABASE_URL and SUPABASE_KEY:
        sb_ok = _sb_save(key, wl)
        if not sb_ok:
            print(f'[watchlist] Supabase保存失敗 → ローカルファイルに保存: {fpath}')
    # ローカルファイルにも常に書く（Supabase障害時の保険 / セッション内バックアップ）
    try:
        with open(fpath, 'w', encoding='utf-8') as f:
            json.dump(wl, f, ensure_ascii=False)
    except Exception as e:
        print(f'[watchlist] ローカル保存エラー: {e}')
...
MACDの数値説明にとどまらず、「なぜこのトレンドが生まれているか」を業績・ニュース・業界の観点から具体的に述べてください。知っている限り最新の情報を積極的に活用してください。"""
    ...
    _GEMINI_MODELS = [
        'gemini-2.0-flash-lite',   # 無料枠: 1500回/日
        'gemini-2.0-flash',         # 無料枠: 1500回/日
        'gemini-2.5-flash',         # 新モデル
        'gemini-2.5-flash-lite',    # 新モデル（軽量）
    ]
    payload = {
...
def debug_watchlist_check():
    """Supabaseのウォッチリスト保存状態を診断する"""
    profile = _req_profile()
    key = _pkey('watchlist', profile)
    result = {
        ...
        'local_file_path': _pfile(WATCHLIST_FILE, profile),
    }
    if SUPABASE_URL and SUPABASE_KEY:
        result['supabase_value'] = _sb_load(key)
    fpath = _pfile(WATCHLIST_FILE, profile)
    if os.path.exists(fpath):
        try:
            with open(fpath, 'r', encoding='utf-8') as f:
